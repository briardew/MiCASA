...
def regrid(dtnow, **kwargs):
    ...
    coordsxyt = {'time': coords['time'], 'lat': coords['lat'], 'lon': coords['lon']}
    ...
    for ff in filescov:
        ...
        ino = np.logical_or.reduce(
            (pbarehi.values > 100, pherbhi.values > 100, ptreehi.values > 100)
        )
        ...
        fbarein = pbarehi.coarsen(x=2, y=2).mean().values.T / 100.0
        ...
        fbaregran = np.histogram2d(LAok, LOok, bins=bins, weights=fbarein[iok])[0]
        fherbgran = np.histogram2d(LAok, LOok, bins=bins, weights=fherbin[iok])[0]
        ftreegran = np.histogram2d(LAok, LOok, bins=bins, weights=ftreein[iok])[0]
