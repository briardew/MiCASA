...
warning off
total = FDC + DF;
dleafpool       = (FDC .* dleafpool     + DF.* leafpool)                                                                           ./ total;
dfrootpool      = (FDC .* dfrootpool    + DF.* frootpool)                                                                          ./ total;
dcwdpool        = (FDC .* dcwdpool      + DF.* (cwdpool + nonCombusted_abovewood))                                                 ./ total;
dsurfmetpool    = (FDC .* dsurfmetpool  + DF.* (surfmetpool + nonCombusted_leaf .* metabfract))                                    ./ total;
dsurfstrpool    = (FDC .* dsurfstrpool  + DF.* (surfstrpool + nonCombusted_leaf .* (1 - metabfract)))                              ./ total;
dsurfmicpool    = (FDC .* dsurfmicpool  + DF.* surfmicpool)                                                                        ./ total;
dsoilmetpool    = (FDC .* dsoilmetpool  + DF.* (soilmetpool + (nonCombusted_froot + nonCombusted_belowwood) .* metabfract))        ./ total;
dsoilstrpool    = (FDC .* dsoilstrpool  + DF.* (soilstrpool + (nonCombusted_froot + nonCombusted_belowwood) .* (1 - metabfract)))  ./ total;
dsoilmicpool    = (FDC .* dsoilmicpool  + DF.* soilmicpool)                                                                        ./ total;
dslowpool       = (FDC .* dslowpool     + DF.* slowpool)                                                                           ./ total;
darmoredpool    = (FDC .* darmoredpool  + DF.* armoredpool)                                                                        ./ total;
warning on

% if no deforestation has occurred yet then total is 0 and the pools become
% NaN, convert these to zero
for pool = 1:length(defoPoolNames)
    eval([defoPoolNames{pool},'(total==0) = 0;'])
end
