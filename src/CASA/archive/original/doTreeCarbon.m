...
% combustion completeness (CC), and tree mortality rate 
warning off
BF = BAwood ./ (gridArea.*FTC);    % convert burned area to fraction of wooded land
BF(BF>1)=1; BF(BF<0)=0; BF(isnan(BF)) = 0;
...
%% DEFORESTATION FIRES
warning off;    DF = BAdefo ./ (FTC .* gridArea);        % convert burned fraction of whole grid cell to fraction forested part
DF(DF>1)=1; DF(isnan(DF)) = 0;
...
%% FUELWOOD collection
if year == spinUpYear && step == 1;   fuelshortage = zeros(size(fuelwooddemand),'single');  end
