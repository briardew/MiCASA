...
% Convert burned area to fraction vegetation burned (same for herbaceous and deforested)
% bweir: what if BAherb > 0 and FHC + FDC = 0? FIXME
BF = zeros(size(BAherb));
io = 0 < FHC + FDC;
BF(io) = BAherb(io) ./ (gridArea(io) .* (FHC(io)+FDC(io)));
BF = min(1, max(0, BF));
