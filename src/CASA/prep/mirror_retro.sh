...
ssh "${HOSTS[0]}" "
    ...
    screen -L -dmS modvir bash --login -c \"echo $HOSTNAME;modvir vegind --mode get --ver "$VERSION" --output "$MIROOT/$DATADIR" --beg $year-01-01 --end $year-12-31\"
    exit"
...
for year in $(seq "$YEAR0" "$YEARF"); do
    ...
    ssh "${HOSTS[$nn]}" "
        ...
        screen -L -dmS modvir bash --login -c \"echo $HOSTNAME;modvir burn --mode get --ver "$VERSION" --output "$MIROOT/$DATADIR" --beg $year-01-01 --end $year-12-31\"
        exit"
done
