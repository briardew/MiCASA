...
for year in $(seq "$YEAR0" "$YEARF"); do
    ...
    ssh "${HOSTS[$nn]}" "
        ...
        screen -L -dmS modvir bash --login -c \"echo $HOSTNAME;modvir vegind --mode regrid --ver "$VERSION" --output "$MIROOT/$DATADIR" --beg $year-01-01 --end $year-06-30\"
        exit"
    ssh "${HOSTS[$nn]}" "
        . "$PREPDIR"/setup.sh
        mkdir -p "$MIROOT"/logs/vegpre/${year}b
        cd "$MIROOT"/logs/vegpre/${year}b || exit
        screen -L -dmS modvir bash --login -c \"echo $HOSTNAME;modvir vegind --mode regrid --ver "$VERSION" --output "$MIROOT/$DATADIR" --beg $year-07-01 --end $year-12-31\"
        exit"
done
